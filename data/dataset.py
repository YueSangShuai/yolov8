...
class ClassificationDataset:
    ...
    def __init__(self, root, args, augment=False, prefix=""):
        ...
        if root.endswith('.txt'):
            if augment==True:
                self.samples = self.load_samples_from_txt(root)
                self.root=os.path.split(root)[0]+"_train"
            else:
                self.samples = self.load_samples_from_txt(root)
                self.root=os.path.split(root)[0]+"_val"
        else:
    ...
    def load_samples_from_txt(self, txt_path):
