...
class Half_finger:
    ...
    def get_ROI(self,img):
        ...
        img = cv2.adaptiveThreshold(img, 210, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 11, 10)
        ...
        try:
            ...
            [RX, RY, RW, RH] = rect
            roi = orign[RY:RY + RH, RX:RX + RW]
            
            return roi,True
        except:
            return orign,False

    def random_erase_continuous(self, image, keep_percentage=0.2):
        ...
        # 计算要保留的区域面积
        keep_area = int(total_area * keep_percentage)

        # 随机生成保留区域的宽度和高度
        keep_width, keep_height = 0, 0
        while keep_width * keep_height < keep_area:
            keep_width = np.random.randint(1, width + 1)
            keep_height = np.random.randint(1, height + 1)

        # 确保保留区域的面积不超过目标面积
        if keep_width * keep_height > keep_area:
            aspect_ratio = keep_width / keep_height
            if keep_width > keep_height:
                keep_width = int(np.sqrt(keep_area * aspect_ratio))
                keep_height = int(keep_area / keep_width)
            else:
                keep_height = int(np.sqrt(keep_area / aspect_ratio))
                keep_width = int(keep_area / keep_height)

        # 随机选择保留区域的位置
        top_left_x = np.random.randint(0, width - keep_width + 1)
        top_left_y = np.random.randint(0, height - keep_height + 1)

        # 创建全白背景图像
        white_background = np.ones_like(image) * 255

        # 在全白背景图像上绘制保留区域
        white_background[top_left_y:top_left_y + keep_height, top_left_x:top_left_x + keep_width] = image[top_left_y:top_left_y + keep_height, top_left_x:top_left_x + keep_width]
